...
def calculate_auc_iou_fpr(gt_folder, pred_folder):
    ...
    for i in tqdm(range(num_images)):
        ...
        pred_path = os.path.join(pred_folder, 'sam_'+gt_files[i])
        # print(gt_files[i], pred_files[i])

        # Preprocess GT and pred images
        gt_img_normalized = preprocess_gt(gt_path)
        pred_img_normalized = preprocess_pred(pred_path)
        # print(pred_img_normalized)
        gt_img_normalized = trans.Scale((256, 256), interpolation=Image.NEAREST)(gt_img_normalized)
        # pred_img_normalized = trans.Scale((256, 256), interpolation=Image.NEAREST)(pred_img_normalized)


        pred_img_normalized = np.array(pred_img_normalized)/255.0
        gt_img_normalized = np.array(gt_img_normalized)/255.0

        # print(pred_img_normalized.shape)
        # print(gt_img_normalized.shape)
        # print(np.max(pred_img_normalized))
        # Flatten the images to 1D arrays
        gt_flat = (gt_img_normalized.flatten() > 0.5).astype(int)
...
if __name__ == "__main__":
    gt_folder_path = './Data/def_gt/'  # Replace with the path to the folder containing GT images
    pred_folder_path = './res/def_image/'  # Replace with the path to the folder containing pred images
