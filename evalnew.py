...
def calculate_auc_iou_fpr(gt_folder, pred_folder):
    ...
    for i in range(num_images):
        ...
        if pred_img_normalized.size != gt_img_normalized.size:
            gt_img_normalized = gt_img_normalized.resize(pred_img_normalized.size, Image.NEAREST)
            
        pred_img_normalized = np.array(pred_img_normalized)/255.0
        gt_img_normalized = np.array(gt_img_normalized)/255.0
        # print(np.min(pred_img_normalized))
        # print(np.max(pred_img_normalized))
        # print(np.min(gt_img_normalized))
        # print(np.max(gt_img_normalized))
...
if __name__ == "__main__":
    gt_folder_path = './Data/t_gt/'  # Replace with the path to the folder containing GT images
    pred_folder_path = './res/'  # Replace with the path to the folder containing pred images
