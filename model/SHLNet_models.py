...
class SHLNet(nn.Module):
    def __init__(self, channel=32):
        super(SHLNet, self).__init__()
        ...
        self.predictor = SamPredictor(self.sam)  # 调用预测模型
        self.img_size = 256
    ...
    def forward(self, x, samx):
        ...
        x_skip = [x1_DSWSAM_1, x2_SWSAM_2, x3_SWSAM_3, x4_SWSAM_4]
        # x2_new = torch.cat((x2_SWSAM_2, self.down_conv1(x1_DSWSAM_1)), 1)
        # x2_new = self.enc_concat(x2_new)
        # x3_new = torch.cat((x3_SWSAM_3, self.down_conv2(x2_new)), 1)
        # x3_new = self.enc_concat2(x3_new)
        # x4_new = torch.cat((x4_SWSAM_4, self.down_conv3(x3_new)), 1)
        # x4_new = self.enc_concat3(x4_new)
        ...
        # print(samx.shape)
        # print( samx.shape[2:])
        self.predictor.set_torch_image(samx, samx.shape[2:], x_skip)
